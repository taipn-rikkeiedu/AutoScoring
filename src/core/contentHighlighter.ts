// src/core/contentHighlighter.ts - Highlights rows on Rikkei LMS pages dynamically
...
export function highlightLmsTableRows() {
...
export function scrollToElementBelowPopup(element: HTMLElement) {
  ...
  if (scrollContainer === window) {
    ...
    const targetScrollTop = relativeElementTop - (container.clientHeight - 100);
    container.scrollTo({
      top: Math.max(0, targetScrollTop),
      behavior: 'smooth'
    });
