...
class AIServiceTest(unittest.TestCase):
    ...
    @patch("config.settings.Settings.DEEPSEEK_API_KEY", "")
    ...
    @patch("config.settings.Settings.DEEPSEEK_API_KEY", "dummy-key")
    ...
    def test_validation_passes_when_deepseek_key_provided_in_config(self):
        # Should not raise ValueError even if settings has no key
        with patch("config.settings.Settings.DEEPSEEK_API_KEY", ""):
            service = AIService(config={"provider": "deepseek", "api_key": "user-key"})
            self.assertEqual(service.api_key, "user-key")

    @patch("config.settings.Settings.OPENROUTER_API_KEY", "")
    ...
    @patch("config.settings.Settings.OPENROUTER_API_KEY", "dummy-key")
