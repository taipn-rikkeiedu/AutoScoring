...
1. **Cập nhật phiên bản ở 2 nơi chính**:
   - [package.json](file:///s:/WorkSpace/RikkeiEducation/REduX/AutoScoring/package.json): Cập nhật trường `"version"`.
   - [constants.ts](file:///s:/WorkSpace/RikkeiEducation/REduX/AutoScoring/src/core/constants.ts): Cập nhật `APP_INFO.version` (nguồn chính nạp version cho manifest của WXT Chrome Extension).

2. **Ghi nhận thay đổi vào Changelog & Nhật ký sửa lỗi**:
   - Thêm thông tin mô tả chi tiết các tính năng hoặc sửa lỗi vừa thực hiện vào [CHANGELOG.md](file:///s:/WorkSpace/RikkeiEducation/REduX/AutoScoring/CHANGELOG.md) dưới mục phiên bản mới theo định dạng hiện tại.
